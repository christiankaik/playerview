...
final class Player {
    ...
    var time: AnyPublisher<TimeInterval?, Never> {
        itimeSubject.map { $0?.seconds as TimeInterval? }
            .eraseToAnyPublisher()
    }
    ...
    private let itimeSubject = CurrentValueSubject<CMTime?, Never>(nil)
...
extension Player {
    ...
    func seek(to seconds: TimeInterval) {
        ...
        let time = CMTime(seconds: seconds, preferredTimescale: item.duration.timescale)
